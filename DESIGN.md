...
- `web/assets/fonts/v2/` contains immutable licensed sources, retained licenses, the explicit Latin glyph manifest, and the pinned reproducible build manifest. Generated production subsets live at stable `/fonts/v2/` URLs, while `web/lib/public-font-assets.ts` records their delivery role, byte size, and SHA-256.
- The localized public layout alone imports route-local `fonts.css` and emits the two typed critical preloads. Next.js automatic font preloads remain disabled because its production manifest hoists them across independent root route groups; deferred IBM Plex Mono faces are never preloaded, and Tracker receives no Living Commons font reference or byte.
- `uv run --frozen python scripts/build_public_fonts.py` rebuilds every WOFF2 subset byte-for-byte with pinned FontTools inputs. `npm --prefix web run check:font-budgets` enforces 160 KiB critical, 220 KiB total, two critical requests, verified hashes, `font-display: swap`, metric fallbacks, and zero Tracker source references; the production isolation and slow-font browser lanes verify emitted resources and font-attributable CLS no greater than 0.02.
