...
The current contract fakes implement the existing `PublicationEffectAdapter` and `JobQueue` ports.
OCR, evidence, and search fakes follow when T2, T3, and T5 introduce those production protocols; the
testkit deliberately does not invent test-only interfaces that application code cannot implement.
